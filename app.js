...
(function () {
  ...
  function clampActiveScroll() {
    const sc = document.querySelector(".view.is-active");
    if (!sc) return;
  ...
  function setVars() {
    ...
    const h = vv && Number.isFinite(vv.height) ? Math.round(vv.height) : Math.round(window.innerHeight);
    root.style.setProperty("--app-h", h + "px");
    const kb = vv && Number.isFinite(vv.height) ? Math.max(0, Math.round(window.innerHeight - vv.height - (vv.offsetTop || 0))) : 0;
  ...
  window.addEventListener("orientationchange", () => setTimeout(setVars, 50), { passive: true });
